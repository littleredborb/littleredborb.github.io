...
function potion(x,y,width,height,effectImg, damage, multiplier, bottle, t, type){
	...
	this.crack.src = 'sounds/crack2.mp3';
...
function craftPotion(dmg, m, EImg, Img, t, type){
	...
	temppot = new potion(0,0,20,20,EImg, dmg, m, Img, t, type);	
}
...
function throwpotion(x){
	...
	//console.log(player1.potions[x].d);

	var shotrange = Math.min(player1.potions[x].d,200);
...
function getCombi(a, b, c, d){
	...
	if(b == true){
		if(c == true){
			...
			craftPotion(0,0,EbImg,sBImg,false,"fullHeal");
			player1.eye--;
			player1.choc--;
			coolDownTime = 200;
			currCoolDown = coolDownTime;

...
var step = new Audio("sounds/s.mp3");
	step.loop = false;
	step.volume = 0.2;
var clicky = new Audio("sounds/click.mp3");
	clicky.loop = false;
var vac = false;
var poofImg = document.createElement('img');
	poofImg.src = 'img/poof.png';
...
function manageFriends(){
	for(var i=0; i<friends.length; i++){
		if(friends[i].timer >0){
			...
			poof = new AnimatedSprite('img/poof.png',75,75,4);
			poofs.push(new poofObject(poof,friends[i].x,friends[i].y));
			friends.splice(i,1);
		}
...
function manageZombie(){
	...
	for(var i=0; i<zombies.length; i++){
		...
		if(zombies[i].currHp<= 0){
			killed = zombies[i];
			poof = new AnimatedSprite('img/poof.png',75,75,4);
			poofs.push(new poofObject(poof,zombies[i].x,zombies[i].y));
			zombies.splice(i,1);
			kills++;
		}
	}
...
function managePoofs(){
	for (var i = 0; i < poofs.length; i++) {
		poofs[i].poof.update(150);
		poofs[i].poof.draw(context,poofs[i].x,poofs[i].y,75,75,1,1);
		if(poofs[i].poof.frame==3){
			poofs.splice(i,1);
		}
	}
